...
interface GameOfflineProps {
  config: {
    ...
    defenseDraws: number;
    legendBurn: number;
    customMaxBonusValue: number;
    customGameMode: "time" | "rounds";
    customWinningGoals: number;
    customTotalRounds: number;
    customHalfTimeBreakDuration: number;
  };
  onReturnToMenu: () => void;
}

export default function GameOffline({ config, onReturnToMenu }: GameOfflineProps) {
      ...
      setTurnTimeLeft((prev) => {
        if (prev <= 1) {
          ...
          if (phase === "player_turn") {
            ...
            if (isMultiplayer) {
              addLog("⏰ انتهى زمن دور المنافس! تم نقل الدور إليك.", "success");
              if (multiplayerRole === "host") {
                ...
                const nextLogs = [
                  ...logs,
                  {
                    id: Math.random().toString(),
                    timestamp: getFormattedTime(),
                    text: "⏰ انتهى زمن دور المنافس! قام النظام بنقل دور اللعب إليك تلقائياً.",
                    type: "success" as const
                  }
                ];
                setLogs(nextLogs);
  ...
  useEffect(() => {
    handleStartGame(
      ...
      config.customLegendPercentage,
      config.maxDraws,
      config.maxMoves,
      config.initialCards,
      config.selectedPlayerPkgs,
      config.selectedSpecialPkgs,
      config.defenseDraws,
      config.legendBurn,
      config.customMaxBonusValue,
      config.customGameMode,
      config.customWinningGoals,
      config.customTotalRounds,
      config.customHalfTimeBreakDuration
    );
  }, [config]);
    ...
    <div style={rotatedStyle} className={mainDivClass}>
      ...
      {phase === "menu" && !isGameLoading && !gameLoadError ? (
        <div className="flex flex-col justify-center items-center flex-1 py-12 px-6 bg-[#0c0d0c]/85 border border-white/5 rounded-2xl backdrop-blur-md text-center max-w-md mx-auto my-12 space-y-4 shadow-2xl">
          ...
          <h3 className="text-lg font-black text-emerald-400">جاري تحميل اللقاء...</h3>
        </div>
      ) : phase === "game_over" ? (
        <GameOverScreen
